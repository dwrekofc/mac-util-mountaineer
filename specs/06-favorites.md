...
## Notes
- **Duplicate rejection implemented** `[RESOLVED P1]`: Was: `add_or_update_share()` performed upsert. Now `add_share` rejects duplicates — users edit `config.toml` directly to change connection details.
