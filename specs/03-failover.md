...
- **No explicit FB-reachable pre-check before unmounting TB** `[observed from code]`: The reconcile logic in `engine.rs` checks `other_reachable` before initiating a switch, which correctly verifies FB is reachable before unmounting TB. However, no single-attempt Fallback retry is implemented — if the mount call fails, the state is left as-is until the next reconcile cycle. The spec's "retry once" requirement is not yet implemented in code.
- **Benign mount collision handling** `[observed from code]`: `is_benign_mount_collision()` detects "file exists" errors combined with AppleScript errors and treats them as non-fatal. This handles cases where macOS already has the volume mounted at the expected path.
